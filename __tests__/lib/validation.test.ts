        ...
        fc.property(
          fc.record({
            ...
            category: fc
              .string()
              .filter(
                s => !VALID_EXERCISE_CATEGORIES.includes(s as ExerciseCategory)
              )
          }),
          data => {
        ...
        fc.property(
          ...
          (exercise: Partial<EnhancedExercise>) => {
            ...
            if (
              ...
              expect(result.isValid).toBe(false)
              expect(result.errors.some(e => e.field === 'muscleGroups')).toBe(
                true
              )
            }
          }
      ...
      fc.assert(
        fc.property(
          ...
          ({ name, currentId, collection }) => {
            ...
            if (duplicate) {
              expect(result.errors.some(e => e.code === 'DUPLICATE_NAME')).toBe(
                true
              )
            }
          }
