...
import type { ExerciseCategory, Program, ProgramBlock, ProgramSession } from '@/types';
import type {
    EnhancedExercise,
    EnhancedProgram,
    FieldValidation
...
const invalidNameArb = fc.string({ minLength: 1, maxLength: 50 })
...
const validEnhancedExerciseArb: fc.Arbitrary<Partial<EnhancedExercise>> = fc.record({
  id: fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0).map(s => s.trim()),
  name: validNameArb,
  ...
  lastUsed: fc.option(fc.constant('2024-01-01T00:00:00.000Z'), { nil: undefined }),
...
const programBlockArb: fc.Arbitrary<ProgramBlock> = fc.oneof(
  ...
  fc.record({
    ...
    exerciseId: fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0).map(s => s.trim()),
    targetReps: fc.option(fc.integer({ min: 1, max: 1000 }), { nil: undefined }),
    durationSeconds: fc.option(fc.integer({ min: 1, max: 3600 }), { nil: undefined }),
...
const validEnhancedProgramArb: fc.Arbitrary<Partial<EnhancedProgram>> = fc.record({
  id: fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0).map(s => s.trim()),
  name: validNameArb,
  description: fc.option(fc.string({ maxLength: 1000 }), { nil: undefined }),
  sessions: fc.array(programSessionArb, { minLength: 1, maxLength: 10 }),
  difficulty: fc.option(validDifficultyArb, { nil: undefined }),
  estimatedDuration: fc.option(fc.integer({ min: 1, max: 300 }), { nil: undefined }),
  tags: fc.option(validStringArrayArb, { nil: undefined }),
  usageCount: fc.option(fc.nat(), { nil: undefined }),
  lastUsed: fc.option(fc.constant('2024-01-01T00:00:00.000Z'), { nil: undefined }),
  createdAt: fc.constant('2024-01-01T00:00:00.000Z'),
...
describe('Validation Infrastructure Property Tests', () => {
  describe('Property 26: Input validation consistency', () => {
    ...
    it('should consistently validate valid exercise data', () => {
      fc.assert(fc.property(validEnhancedExerciseArb, (exercise) => {
    ...
    it('should consistently validate exercise categories', () => {
      fc.assert(fc.property(
        fc.record({
          name: validNameArb,
          category: fc.string().filter(s => !VALID_EXERCISE_CATEGORIES.includes(s as ExerciseCategory))
        }),
        (data) => {
    ...
    it('should consistently validate program structure', () => {
      fc.assert(fc.property(validEnhancedProgramArb, (program) => {
    ...
    it('should consistently validate array fields', () => {
      fc.assert(fc.property(
        ...
        (exercise: Partial<EnhancedExercise>) => {
          ...
          if (exercise.muscleGroups && exercise.muscleGroups.some(g => 
            typeof g !== 'string' || (typeof g === 'string' && g.trim().length === 0)
          )) {
            expect(result.isValid).toBe(false);
            expect(result.errors.some(e => e.field === 'muscleGroups')).toBe(true);
          }
        }
      ), { numRuns: 100 });
    });
      ...
      fc.assert(fc.property(
        ...
        (data) => {
          ...
          if (data.usageCount !== undefined && data.usageCount < 0) {
            ...
            expect(exerciseResult.errors.some(e => e.field === 'usageCount')).toBe(true);
          }
          ...
          if (data.estimatedDuration !== undefined && data.estimatedDuration <= 0) {
            ...
            expect(programResult.errors.some(e => e.field === 'estimatedDuration')).toBe(true);
          }
        }
      ), { numRuns: 100 });
    });
    ...
    it('should consistently validate dependency checking', () => {
      fc.assert(fc.property(
        ...
        ({ exerciseId, programs }) => {
          const result = checkExerciseDependencies(exerciseId, programs as Program[]);
    ...
    it('should consistently validate unique names', () => {
      fc.assert(fc.property(
        fc.record({
          name: fc.string({ minLength: 1 }),
          currentId: fc.option(fc.string({ minLength: 1 }), { nil: undefined }),
          collection: fc.array(fc.record({
            id: fc.string({ minLength: 1 }),
            name: fc.string({ minLength: 1 })
          }))
        }),
