...
| Exercise pagination                 | Infinite scroll via `FlatList.onEndReached`            | `fetchExercises()` supports paginated access with `page` and `limit` params, returning `PaginatedResponse<Exercise>` (`{ data, pagination }`). The exercise library implements infinite scroll — DataContext fetches page 1 on auth, then `loadMoreExercises()` appends subsequent pages as the user scrolls. After exercise create/edit/delete, state resets and re-fetches from page 1. Same pattern for `fetchExercisesByCategory()`. |
